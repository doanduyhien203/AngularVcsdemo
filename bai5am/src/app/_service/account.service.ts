...
import { SortColumn, SortDirection } from '../account/sortable.directive';
...
function matches(user: User, term: string, pipe: PipeTransform) {
  return (
    ...
    pipe.transform(user.account_number).includes(term) ||
    pipe.transform(user.balance).includes(term)
  );
}
